...
const navItems: NavEntry[] = [
  {
    ...
    items: [
      ...
      { title: "Agents", href: ROUTES.agents, icon: MessageSquare, visible: isAdmin },
      {
        ...
        icon: Server,
        items: [
          { title: "Connections", href: ROUTES.llmConnections },
    ...
    <SidebarFooter className="border-sidebar-border border-t group-data-[collapsible=icon]:hidden">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="flex w-full items-center gap-2 rounded-md p-1.5 text-left transition-colors hover:bg-sidebar-accent">
            <Avatar size="sm">
              <AvatarFallback className="text-[11px]">{initials}</AvatarFallback>
            </Avatar>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium leading-none">{user.name}</p>
              <p className="text-sidebar-foreground/50 truncate text-xs capitalize">{user.role}</p>
            </div>
          </button>
        </DropdownMenuTrigger>

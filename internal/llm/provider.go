...
// (ConfigurationSchema) that validates the config map passed to the provider.
// Key is the registry identity for the provider (see R.Register) and must be
// set by whoever builds the provider (e.g. via a CreateProvider's WithKey
...
// Provider.Stream: the Prompt to send and the Tools the model may call.
